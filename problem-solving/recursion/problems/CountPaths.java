 ...
 */
 ...
 public class CountPaths {
  ...
  public int countPaths(int m, int n, int x, int y) {
    ...
    return helper(m, n, x, y);
  }
  private int helper(int m, int n, int x, int y) {
    if (x == 0 && y == 0) {
      return 1; // Reached (0, 0);
    }
    if (x < 0 || y < 0) {
      return 0; // Out of bound
    }
    // From every step we can either go to up or left, as these are only two possible ways to get closer to 0, 0
